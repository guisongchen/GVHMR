...
表面看 COCO-17 中脸部占 5/17（≈29%）观测维度，而 SMPLX `body_pose` 头部相关只有 neck + head 2 个关节，似乎"过剩"。但信息价值取决于约束了哪些输出自由度，而非数量对应：

1. **脸部 5 点约束的是头部朝向（6 DOF）**：neck + head 各 3DOF 旋转。单点只能定位置，朝向完全不可观（低头/转头时 2D 投影位置几乎不变）；鼻+双眼+双耳的构型编码 yaw/pitch/roll——10 个观测数约束 6 个自由度，刚好留有余量。头部朝向还携带运动意图的间接证据（人往哪看与往哪走高度相关）。
2. **obs 进 Transformer 是全局混合而非 1:1 映射**：网络学的是"17 点联合构型 → 全身姿态"，脸部点参与全身推理（如侧脸+肩线组合判断侧身程度），冗余是抗遮挡的投票机制。
3. **计算零成本**：17 个热图一次前向同时输出，裁掉脸部点不省任何耗时，反而需重训姿态模型。

**真正的观测/输出自由度错配在手和脚**：

| 部位 | COCO-17 观测 | SMPLX 输出自由度 |
|---|---|---|
| 脸/头 | 5 点（≈10 维） | 2 关节（6 DOF）——基本匹配 |
| **手** | **0 点** | 2×15 关节（**30 DOF，完全靠 AMASS 先验脑补**） |
| **脚尖/脚跟** | **0 点**（只有踝） | 足部关节 + 接触判断（static_conf 只能靠踝间接推） |

🔧 **升级方向**：换 WholeBody 格式的姿态估计器（COCO-WholeBody 133 点：含 2×21 手部 + 脚部 + 68 点面部，如 DWPose/RTMW 系列），补齐手、脚观测缺口。代价：`obs` 维度 17×3 → 133×3，需改 `relative_transformer.py` 的 obs embedder + 训练侧噪声增广与 GT 投影格式 → **触发 GVHMR 重训**，属研究级改动。若立项，建议与 SAM3 提案一样先在 `docs/proposals/` 单独立档评估。
...
#### 📌 为什么输出两套 SMPLX 参数（incam vs global）？

两套参数**共享 `body_pose` 与 `betas`**，差别只在 `global_orient` 和 `transl`——"人相对坐标系原点的朝向与位置"有两种合法表达：

| | `smpl_params_incam` | `smpl_params_global` |
|---|---|---|
| 坐标系 | **相机系**（原点=光心，随相机移动） | **世界系 / GV 系**（原点固定在地面，y 轴对齐重力） |
| `transl` 来源 | `pred_cam` + `bbx_xys` + K 反投影（`compute_transl_full_cam`） | 网络预测 GV 系朝向 + 局部速度，与 `cam_angvel` 复合后 roll-out 积分（`get_smpl_params_w_Rt_v2`） |
| 回答的问题 | "画面里的人在相机前怎么动" | "人在真实世界里怎么走" |
| 典型用途 | 渲染叠回原视频（`1_incam.mp4`）、相机系评测指标（PA-MPJPE/PVE） | 地面轨迹（`2_global.mp4`）、动捕、机器人模仿等下游应用 |

**只输出一套行不行？**

- 只有相机系：相机运动混入 `transl`，无法区分"人走 5 米"还是"相机摇 5 度"，一切需要地面真值的下游任务不可做；
- 只有世界系：缺相机外参无法投影回原视频（渲染不可能）；逐帧姿态评测不需要世界轨迹；且世界系朝向只能经积分间接约束，监督更弱（训练 loss 中 `spv_incam_only` mask 就是为 3DPW 这类仅相机系标注的数据准备的）。

**为什么网络显式预测两条路径而非"预测一套再换算"：**

1. **误差形式不同**：相机系姿态由逐帧观测直接约束，精度高但无世界锚点；GV 轨迹沿时间积分会累积误差——直接预测"局部速度"并让数据监督积分结果，比"逐帧估相机姿态再机械复合"更鲁棒（VO 只给旋转，平移本就缺失）；
2. **解耦学习**：GV 表示把"身体怎么动"（`local_transl_vel`，与相机无关）和"相机怎么动"（`cam_angvel`）显式分开，网络学习物理上更干净的量，泛化更好；
3. **后处理需要**：静止关节修正（`pp_static_joint`，消除脚底打滑）必须在世界系进行——"脚相对地面静止"的约束在相机系里不成立。

#### ⚠️ 两套坐标系之间不满足严格换算关系

**结构上就无法写出严格换算式。** 严格换算需要每帧完整相机外参 `X_cam = R_w2c·X_world + t_w2c`，但：旋转 `R_w2c` 由 VO 提供 ✅；平移 `t_w2c` 从未被估计（SimpleVO 只输出旋转，世界系 `transl` 完全来自 `local_transl_vel` 积分）❌。

**两者实际是"软连接"**（`get_smpl_params_w_Rt_v2`，`gvhmr_pipeline.py:322`）：

1. 相机在世界中的姿态**不是输入而是反推**：`R_c2gv = R_gv @ R_cᵀ`——`global_orient_gv` 与 `global_orient_c` 是网络独立预测的两个量，每帧相机姿态是它们的"商"；
2. `cam_angvel` 只做弱约束：取 `R_c2gv` 的视轴、**抹掉 y 分量**（强制重力对齐、只留水平朝向）、预测相邻帧视轴变化、**高斯平滑**后积分出 `R_t_to_0`；
3. 每个环节（只留水平分量、平滑、积分）都在牺牲严格性换取稳定性——连接两套输出的不是刚体变换，而是"只约束偏航角变化率、其余交给数据监督"的概率性纽带。

**实际后果（二次开发必须知道）**：

- 两套输出**可以互相矛盾**：global 用 VO 外参重投影回画面 ≠ incam 渲染结果（前者平滑但会漂，后者逐帧准但无世界锚点）；
- **误差分配不对称**：incam 误差逐帧独立；global 误差沿时间积分累积（静止关节修正只在 global 上做，部分原因即在此）；
- 后处理进一步打破对称：`process_ik` 改的 `body_pose` 两边共享，`transl` 修正只作用于 global。

**为什么故意这样设计**：严格耦合会把 VO 误差硬传导进姿态预测（VO 旋转错 2°，世界朝向必须跟着错）。解耦后网络可从图像证据学习"VO 不可信时信什么"，VO 退化为变化率先验——与 `obs` 的"测量通道 + 动作先验"同一哲学：**所有外部信号都是带噪声的条件，而非硬约束**。

> 🔧 **改造方向**：若下游需要严格自洽的两套参数（多视角合成、场景级重建、与 LiDAR 点云对齐），需补齐相机平移估计（换 DPVO 或接外部 SLAM）并加一致性约束。这是一个与 SAM3 提案同级的候选课题。

#### 📌 两套输出在后续流程中的分工

**incam 是"观测空间"**（贴合像素证据，服务渲染与逐帧评测）；**global 是"产品空间"**（承载物理约束，服务轨迹质量与下游应用）。两者分工几乎不交叉，`body_pose`/`betas` 是唯一共享的桥梁：

| 环节 | incam | global |
|---|---|---|
| **后处理**（§5） | `transl` 保持网络原始输出；`body_pose` 随 IK 同步更新 | `pp_static_joint` 修正 `transl`（"脚相对地面静止"只在世界系成立）；`body_pose` 随 IK 更新 |
| **渲染**（§6） | `1_incam.mp4`：网格 + 真实内参 K 叠回原视频 | `2_global.mp4`：虚拟环绕相机（`get_global_cameras_static`）+ 24mm 虚拟镜头 + 地面网格 |
| **评测**（`hmr4d/utils/eval/eval_utils.py`） | `compute_camcoord_metrics`：PA-MPJPE / MPJPE / PVE / accel（**逐帧姿态精度**） | `compute_global_metrics`：WA-MPJPE / WAA-MPJPE / RTE（root 轨迹误差）/ jitter / foot-sliding（**世界轨迹质量**） |
| **训练监督** | 3DPW 等相机系标注直接监督（`spv_incam_only` mask） | AMASS/BEDLAM 世界系数据监督积分结果 |
| **下游导出** | 视频叠加、AR 效果 | 动捕、机器人 retarget、步态分析、场景合成（配合 `hmr4d/utils/ik/`） |

两个评测细节的出处：

- WA-MPJPE 把序列切成 100 帧 chunk 逐块对齐后再算误差（`eval_utils.py` `compute_global_metrics`，沿袭 WHAM）——容忍积分漂移，只考核窗口内相对运动质量；RTE 则直接惩罚全局轨迹偏差；
- 后处理刻意只改 global 的 `transl`，两套参数的一致性在此**主动松动**——incam 保留观测保真度，global 换取物理合理性。
...
- **作用**: 在「SMPLX 物理参数空间」和「网络归一化特征空间」之间双向转换。网络不直接回归 SMPLX 参数（量纲各异、旋转有奇异性、分布不均），而是输出一个 **151 维归一化向量 `x`**，编解码层负责双向翻译：
...
- 训练时还用 `get_noisyobs` 对姿态加高斯噪声做增强（见 §1.2 的"测量通道"注解）

### 📌 151 维向量的布局（`endecoder.py:102-137`）

| 维度切片 | 内容 | 说明 |
|---|---|---|
| `0:126` | `body_pose_r6d` | 21 关节 × 6D 旋转（axis-angle → 矩阵 → 6D，连续无奇异） |
| `126:136` | `betas` | SMPLX 形状 10 维 |
| `136:142` | `global_orient_r6d` | **相机系**朝向（incam 路径） |
| `142:148` | `global_orient_gv_r6d` | **GV 系**朝向（global 路径）；编码时 `= R_c2gv @ global_orient_R` |
| `148:151` | `local_transl_vel` | 局部坐标系位移速度——**不直接预测 `transl`**，积分（rollout）得轨迹 |

设计要点：

- **z-score 标准化**（减 mean / 除 std）使各维方差为 1，拉平网络各维度的学习难度；
- **旋转一律用 6D 表示**进/出网络（连续、无符号歧义、无万向锁），axis-angle 只在 SMPLX 边界转换；
- 网络因此永远工作在"均值 0、方差 1、连续无奇点"的空间，物理量合法性、量纲、旋转表示由编解码层兜底。

### 📌 附属职责

- 内置 SMPLX 模型提供 **FK**（`fk_v2`：参数 → 关节 3D 位置），后处理与训练 loss 共用；
- `encode_translw` / `decode_translw` 单独处理 `local_transl_vel`（供只训练轨迹分支的实验）；
- `R_c2gv` 参与编码，是双输出设计（§2.4）在编码侧的实现。
...
- **解决的问题**：网络输出有两类系统性瑕疵——**脚底打滑/轨迹漂移**、**姿态与修正后轨迹脱节**。后处理利用网络自己预测的 `static_conf_logits`（每关节"此刻是否与地面相对静止"的置信度）做两步几何修正。
- **监控关节**：`joint_ids = [7, 10, 8, 11, 20, 21]` = 左右踝、左右脚尖、左右腕。

### 📌 第一步：`pp_static_joint` —— 修正世界轨迹（只改 global 的 `transl`）

1. FK 算出世界系关节位置，取 6 个监控关节的相邻帧位移；
2. 若某帧有关节判定为"静止"（`static_conf_logits > 0`），则根节点**本不应产生这些关节的位移**——按 softmax(置信度) 加权求出"不该发生的位移量"；
3. 从 `transl` 逐帧位移中扣除，cumsum 重积分成新轨迹；
4. XZ 方向高斯平滑；整条序列最低点贴到地面（y=0）。

静态相机时换用变体 `pp_static_joint_cam`（利用相机静止先验）。效果：站立时脚不随轨迹噪声滑动，行走时步幅与位移一致。

### 📌 第二步：`process_ik` —— 让姿态匹配修正后的轨迹（改 `body_pose`，两边同步）

轨迹改了但姿态还是网络原输出，脚的位置会与新轨迹脱节：

1. **接触点粘连**（sebas rollout merge）：接触关节目标位置 = 上帧位置×置信度 + 当前帧位置×(1−置信度)——接触中的脚钉在原地；
2. **CCD 逆运动学**：沿四条运动链（左腿 `0→1→4→7→10`、右腿、左右臂至腕）各跑 2 轮 CCD-IK（`hmr4d/utils/ik/ccd_ik.py`），微调链上关节旋转使踝/腕精确到达目标；
3. 修正后关节旋转转回 axis-angle，**同时写回 incam 和 global 的 `body_pose`**（两套输出的 `body_pose` 因此始终一致，见 §2.4）。

### 📌 为什么不用网络自己学好这些？

打滑与 IK 一致性本质是**硬几何约束**（接触点速度=0、关节链长度固定）。端到端学会需要大量数据和容量，几何方法几行代码即可精确满足——典型的"网络给初值 + 几何先验做投影"混合式设计，与全项目"外部信号是带噪条件而非硬约束"的哲学互补：**能解析求解的部分不留给网络**。

> 注：训练时**不做后处理**（`train.py` 路径 `postproc=False`），故训练日志的全局指标与测试脚本略有差异（README 有说明）。
